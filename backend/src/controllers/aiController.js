...
exports.summarize = async (req, res) => {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are an expert academic summarizer. Provide comprehensive yet concise summaries of academic documents using markdown formatting.',
      },
      {
        role: 'user',
        content: `Please provide a comprehensive summary of the following document titled "${ctx.name}" from the course "${ctx.course}":\n\n${ctx.text.slice(0, 15000)}`,
      },
    ],
    max_tokens: 1500,
  });
...
exports.keyPoints = async (req, res) => {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: `Extract the key points, concepts, and important terms from: "${ctx.name}"\n\n${ctx.text.slice(0, 12000)}\n\nReturn JSON: {"keyPoints": ["..."], "concepts": ["..."], "terms": [{"term": "...", "definition": "..."}]}`,
      },
    ],
    max_tokens: 1500,
    response_format: { type: 'json_object' },
